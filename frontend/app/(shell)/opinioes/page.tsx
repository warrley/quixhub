...
interface DisciplineGroup {
  ...
  offerings: OfferingSearchResult[];
}
...
export default function Opinioes() {
  ...
  const groups = useMemo<DisciplineGroup[]>(() => {
    const filtered = disciplineFilter ? results.filter((o) => o.disciplineId === disciplineFilter) : results;
    const byDiscipline = new Map<string, DisciplineGroup>();
    for (const o of filtered) {
      const group = byDiscipline.get(o.disciplineId) ?? {
      ...
      byDiscipline.set(o.disciplineId, group);
    }
    return [...byDiscipline.values()].sort((a, b) => a.disciplineName.localeCompare(b.disciplineName));
            ...
            <Card key={group.disciplineId} padding="md">
              ...
              <div className="flex flex-col gap-2">
                {group.offerings.map((o) => {
                  const s = statsByProfessor.get(o.professor);
                  return (
                    <Link
                      key={o.id}
                      href={`/opinioes/${o.id}`}
                      className="flex items-center justify-between gap-2.5 border border-line rounded-md py-2.5 px-3 no-underline text-inherit hover:bg-surface-sunken"
                    >
                      <div>
                        <div className="font-semibold text-13">{o.professor}</div>
