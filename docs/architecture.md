...
- `frontend/` — Next.js (App Router) + TypeScript + React 19, Tailwind CSS v4 over a token-based theme (`src/styles/tokens.css`, registered via `@theme`, light + dark) — CSS Modules were migrated away entirely in favor of utility classes. Routes live under `app/`, grouped into `(auth)` (Login/Register, no shell) and `(shell)` (everything else, wrapped in `AppShell`): Home, Catálogo, Opiniões, Materiais, Calendário, IRA, Fluxograma, Perfil, plus an admin moderation queue. Most screens still run on `src/data/mock.ts` fixture data; **Opiniões** (`/opinioes`, `/opinioes/[id]`), the **professor+discipline general view** (`/opinioes/professor/[disciplineId]`), and the **Catálogo** list + detail pages are wired to the real backend via `src/lib/api.ts` — see "Data model" below. `/opinioes`, `/opinioes/[id]`, `/catalogo`, and `/catalogo/[id]` are live in production; `/opinioes/professor/[disciplineId]` is still gated behind `UnderConstruction` pending verification.
